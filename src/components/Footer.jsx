...
// import DripDivider from "./DripDivider"; // Unused now
...
export default function Footer() {
  ...
  return (
    <footer style={{ background: "var(--ice-honey)", padding: "60px 32px 40px", borderTop: "1px solid rgba(0,0,0,0.05)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 48, marginBottom: 80 }}>
          <div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" style={{ width: 40, height: 40, borderRadius: "var(--r-md)", background: "rgba(0,0,0,0.05)", border: "1px solid rgba(0,0,0,0.08)", display: "flex", alignItems: "center", justifyContent: "center", textDecoration: "none", transition: "background var(--fast)" }}
                onMouseEnter={e => e.currentTarget.style.background = "rgba(0,0,0,0.1)"}
                onMouseLeave={e => e.currentTarget.style.background = "rgba(0,0,0,0.05)"}
                aria-label={`Instagram social link`}>
                <Instagram size={20} color="rgba(60,47,47,0.6)" />
              </a>
            </div>
          </div>
          {cols.map(c => (
